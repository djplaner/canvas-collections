...
var TAILWIND_CSS = '<link href="https://unpkg.com/tailwindcss@^2/dist/tailwind.min.css" rel="stylesheet">';
var CANVAS_COLLECTIONS_CSS = '<link href="https://djon.es/gu/canvas/canvas-collections.css" rel="stylesheet">';
...
var cc_CanvasModulesView = function () {
     ...
     */
    function cc_CanvasModulesView(modules, collectionsClick) {
    ...
    _createClass(cc_CanvasModulesView, [{
        ...
        key: 'stopCardDescriptionPropagation',
        value: function stopCardDescriptionPropagation() {
            // get all the links in .carddescription that are not .gu-engage
            var links = document.querySelectorAll('.carddescription a:not(.gu-engage)');
        ...
        key: 'makeCardsClickable',
        value: function makeCardsClickable() {

            // get all the clickable cards
            var cards = document.getElementsByClassName('clickablecard');
        ...
        value: function generateNavBar() {
            ...
            var _loop = function _loop(collection) {
                ...
                navItem.onclick = function () {
                    return _this.collectionsClick(collection, _this);
                };
        ...
        key: 'updateProgress',
        value: function updateProgress(module, cardCollection) {
            // no progress if null
            if (module.percentItemsComplete === null) {
                return;
            }

            var valueBackground = "#ccc";
            ...
            var progress = new CircularProgressBar(module.percentItemsComplete, options);
            var card = cardCollection.querySelector('div#cc_module_' + module.id);
            var progressDiv = card.querySelector('div.cc_progress');
            if (progressDiv) {
        ...
        key: 'updateImage',
        value: function updateImage(module, cardCollection) {
            ...
            var card = cardCollection.querySelector('div#cc_module_' + module.id);
            var imageDiv = card.querySelector('div.cc_module_image');
            // remove imageDiv
            if (imageDiv) {
                imageDiv.remove();
            }
...
var cc_Item = function () {
    ...
    _createClass(cc_Item, [{
        ...
        */
        ...
        key: 'extractStatus',
        value: function extractStatus(element) {

            this.status = null;
            this.isComplete = false;
            // get the div.module-item-status
            var statusDiv = element.querySelector('div.module-item-status-icon');

            if (!statusDiv) {
                return;
            }
            // try markAsRead
            var markAsRead = statusDiv.querySelector('i.icon-mark-as-read');
            if (markAsRead) {
                this.status = 'MarkAsRead';
                return;
            }
            // try completed
            var completed = statusDiv.querySelector('i.icon-check');
            if (completed) {
...
var cc_Module = function () {
    ...
    _createClass(cc_Module, [{
        ...
        key: 'calculateItemProgress',
        value: function calculateItemProgress() {
            ...
            var numItemsToComplete = this.items.filter(function (item) {
                return item.status !== null;
            }).length;
            // # of items completed are those with item.isComplete==true
            var numItemsCompleted = this.items.filter(function (item) {
