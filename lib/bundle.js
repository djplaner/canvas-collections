...
exports.canvasCollections = canvasCollections;

var _hello = require("./hello");

const COURSE_ID = ENV.COURSE_ID; //const CSS_URL='<link rel="stylesheet" href="https://s3.amazonaws.com/filebucketdave/banner.js/cards.css" />';
...
const COLLECTIONS_DEFAULTS = ["Learning Journey", "Assessment Essentials", "Online Workshops", "Student Support"];
const META_DATA_FIELDS = ['image', 'label', 'imageSize', 'num', 'description', 'collection'];
const CARD_DEFAULTS = {
  'Welcome': {
...
class cc_CanvasModulesView {
  ...
  constructor(modules, collectionsClick, options = null) {
    ...
    this.currentCollection = this.model.currentCollection; // default setting
  ...
  render() {
    ...
    let canvasContent = document.getElementById('context_modules'); // on a canvas page where there are no modules
    ...
    ccCanvasCollections.appendChild(cards); // insert the collections before canvasContent
    //result = canvasContent.insertBefore(ccCanvasCollections, canvasContent.firstChild);

    const result = canvasContent.insertBefore(ccCanvasCollections, canvasContent.firstChild); // cards are now in the DOM, do some final updates
    // Make the cards clickable

    this.stopCardDescriptionPropagation();
    this.makeCardsClickable(); // Update the titles of the modules to add the collection name
  ...
  stopCardDescriptionPropagation() {
    ...
    let links = document.querySelectorAll('.carddescription a:not(.gu-engage)'); // prevent propagation of the click event on all links
  ...
  makeCardsClickable() {
    ...
    for (let i = 0; i < cards.length; i++) {
      let card = cards[i]; // add the event listener
  ...
  updateCanvasModuleList() {
    ...
    for (let numModule in this.modules) {
      let aModule = this.modules[numModule]; // Find the module's name and it's title dom

      let divDom = document.querySelector(`div#context_module_${aModule.id}`);
      let spanDom = divDom.querySelector('span.name'); // if we found the title, add the collection details
  ...
  generateNavBar() {
    ...
    for (let collection of collections) {
      ...
      let navElement = `
            ...
            `;
      let navItem = this.createElement('li', 'mr-4'); //navItem.onclick = () => cc_collectionClick(collection,this);
  ...
  generateCards() {
    ...
    const numModules = this.modules.length; //        const numRequiredRows = Math.ceil(numModules/3);

    let cardsShown = 0; // for each module generate card and append
    ...
    for (let i = 0; i < numModules; i++) {
      ...
      if (module.collection === this.currentCollection || this.options.collectionView === 'all') {
        ...
        cardCollection.appendChild(card); // once card is added to DOM, can do further updates
    ...
    if (cardsShown === 0) {
      let noCards = this.createElement('div', 'cc-canvas-collections-no-cards'); // add some padding-left
  ...
  generateCard(module) {
    ...
    const DATE = this.generateDateView(module.date); //        let WIDTH="w-full sm:w-1/2 md:w-1/3";
...
`; //</div>
    ...
    let wrapper = this.createElement('div', ['clickablecard', 'w-full', 'sm:w-1/2', 'md:w-1/3', 'flex', 'flex-col', 'p-3']);
  ...
  updateImage(module, cardCollection) {
    ...
    let card = cardCollection.querySelector(`div#cc_module_${module.id}`);
    let imageDiv = card.querySelector('div.cc_module_image'); // remove imageDiv
...
class cc_Item {
  ...
  constructor(element) {
    ...
    this.extractStatus(element); //        console.log(`canvas-collection:    -- ${this.position}) item ${this.id} '<a href="${this.url}">${this.title}</a>' is ${this.itemType}`);
  ...
  extractId(element) {
    this.id = null; // get dom element id

    let id = element.id; // extract id from context_module_item_ + id
  ...
  extractAbout(element) {
    ...
    for (let i = 0; i < aboutSpans.length; i++) {
      let span = aboutSpans[i]; // only if display is none
  ...
  */
  ...
  extractStatus(element) {
    this.status = null;
    this.isComplete = false; // get the div.module-item-status
...
class cc_Module {
  ...
  extractId(element) {
    this.id = null; // attribute data-module-id contains the id
  ...
  extractPublished(element) {
    this.published = true; // the icon can be unreliable
    //let unpublishIcon = element.querySelector('i.icon-unpublish');

    let unpublish = element.querySelector('span.publish-icon'); // is unpublished in the class list
  ...
  calculateItemProgress() {
    ...
    const numItems = this.items.length; // # of items that can be completed are those with item.status!=null

    const numItemsToComplete = this.items.filter(item => item.status !== null).length; // # of items completed are those with item.isComplete==true
...
class cc_CanvasModules {
  constructor() {
    ...
    this.currentCollection = DEFAULT_ACTIVE_COLLECTION; // loop thru each moduleElement and construct a cc_Module object
...
class cc_Controller {
  constructor() {
    ...
    const location = window.location.href; // extract from location everything after ?

    const queryString = location.substring(location.indexOf('?') + 1); // define options
    ...
    this.modules = new cc_CanvasModules(); // update the page to add Card Information
  ...
  collectionClick(collectionName, view) {
    ...
    view.currentCollection = collectionName; // remove div#guCardInterface
...
function canvasCollections() {
  document.head.insertAdjacentHTML('beforeend', TAILWIND_CSS); // Wait for everything to load
